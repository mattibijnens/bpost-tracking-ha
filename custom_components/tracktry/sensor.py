...
class TrackTrySensor(Entity):
    ...
    async def fetch_tracking_object(self, code, postalCode):
        response = await self.hass.async_add_executor_job(requests.get("https://track.bpost.cloud/track/items?itemIdentifier=" + str(code) + "&postalCode=" + str(postalCode)))
        #return json.loads(
